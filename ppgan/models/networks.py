...
def get_norm_layer(norm_type='instance'):
    ...
    """
    if norm_type == 'batch':
        norm_layer = functools.partial(nn.BatchNorm)#, affine=True, track_running_stats=True)
    elif norm_type == 'instance':
        norm_layer = functools.partial(nn.InstanceNorm)#, affine=False, track_running_stats=False)
    elif norm_type == 'none':
...
class ResnetGenerator(paddle.fluid.dygraph.Layer):
    ...
    def __init__(self, input_nc, output_nc, ngf=64, norm_layer=nn.BatchNorm, use_dropout=False, n_blocks=6, padding_type='reflect'):
        ...
        for i in range(n_downsampling):  # add downsampling layers
            mult = 2 ** i
            model += [nn.Conv2D(ngf * mult, ngf * mult * 2, filter_size=3, stride=2, padding=1, bias_attr=use_bias),
...
class ResnetBlock(paddle.fluid.dygraph.Layer):
    ...
    def build_conv_block(self, dim, padding_type, norm_layer, use_dropout, use_bias):
        ...
        conv_block += [nn.Conv2D(dim, dim, filter_size=3, padding=p, bias_attr=use_bias), norm_layer(dim), nn.ReLU(True)]
        if use_dropout:
...
class UnetSkipConnectionBlock(paddle.fluid.dygraph.Layer):
    ...
    def __init__(self, outer_nc, inner_nc, input_nc=None,
        ...
        if input_nc is None:
            input_nc = outer_nc
        downconv = nn.Conv2D(input_nc, inner_nc, filter_size=4,
...
class NLayerDiscriminator(paddle.fluid.dygraph.Layer):
    ...
    def __init__(self, input_nc, ndf=64, n_layers=3, norm_layer=nn.BatchNorm):
        ...
        kw = 4
        padw = 1
        sequence = [nn.Conv2D(input_nc, ndf, filter_size=kw, stride=2, padding=padw), LeakyReLU(0.2, True)]
        nf_mult = 1
        nf_mult_prev = 1
        for n in range(1, n_layers):  # gradually increase the number of filters
            ...
            nf_mult = min(2 ** n, 8)
            sequence += [
                nn.Conv2D(ndf * nf_mult_prev, ndf * nf_mult, filter_size=kw, stride=2, padding=padw, bias_attr=use_bias),
        ...
        sequence += [
            nn.Conv2D(ndf * nf_mult_prev, ndf * nf_mult, filter_size=kw, stride=1, padding=padw, bias_attr=use_bias),
            norm_layer(ndf * nf_mult),
            LeakyReLU(0.2, True)
        ]

        sequence += [nn.Conv2D(ndf * nf_mult, 1, filter_size=kw, stride=1, padding=padw)]  # output 1 channel prediction map
...
class PixelDiscriminator(paddle.fluid.dygraph.Layer):
    """Defines a 1x1 PatchGAN discriminator (pixelGAN)"""

    def __init__(self, input_nc, ndf=64, norm_layer=nn.BatchNorm):
        ...
        self.net = [
            nn.Conv2D(input_nc, ndf, filter_size=1, stride=1, padding=0),
            LeakyReLU(0.2, True),
            nn.Conv2D(ndf, ndf * 2, filter_size=1, stride=1, padding=0, bias_attr=use_bias),
            norm_layer(ndf * 2),
            LeakyReLU(0.2, True),
            nn.Conv2D(ndf * 2, 1, filter_size=1, stride=1, padding=0, bias_attr=use_bias)]
